...
from didcomm.common.utils import extract_key, \
...
async def anoncrypt(msg: bytes,
                    to: DID_OR_DID_URL,
                    alg: AnonCryptAlg,
                    resolvers_config: ResolversConfig) -> AnoncryptResult:

    jwe = JsonWebEncryption()

    to_verification_methods = \
        await find_key_agreement_recipient_verification_methods(to, resolvers_config)
...
async def unwrap_anoncrypt(msg: dict,
                           resolvers_config: ResolversConfig) -> UnwrapAnoncryptResult:
