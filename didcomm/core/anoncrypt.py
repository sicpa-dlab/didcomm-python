...
async def find_keys_and_anoncrypt(
    ...
    to_verification_methods = await find_anoncrypt_pack_recipient_public_keys(to, resolvers_config)
    to_public_keys = [Key(kid=to_vm.id, key=extract_key(to_vm)) for to_vm in to_verification_methods]
...
async def unpack_anoncrypt(
    ...
    unpack_res = None
    async for to_secret in find_anoncrypt_unpack_recipient_private_keys(to_kids, resolvers_config):
