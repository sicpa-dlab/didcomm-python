...
from didcomm.core.keys.authcrypt_keys_selector import find_authcrypt_pack_sender_and_recipient_keys, \
...
def authcrypt(
    ...
    res = jwe.serialize_json(
        header_obj, msg, to_keys, sender_key=frm.key
    )
...
async def find_keys_and_authcrypt(
    ...
    pack_keys = await find_authcrypt_pack_sender_and_recipient_keys(frm, to, resolvers_config)
    frm_private_key = Key(kid=pack_keys.sender_private_key.kid, key=extract_key(pack_keys.sender_private_key))
    to_public_keys = [Key(kid=to_key.id, key=extract_key(to_key)) for to_key in pack_keys.recipient_public_keys]
...
async def unpack_authcrypt(
    ...
    unpack_res = None
    async for unpack_keys in find_authcrypt_unpack_sender_and_recipient_keys(frm_kid, to_kids, resolvers_config):
        frm_public_key = extract_key(unpack_keys.sender_public_key)
        to_private_kid_and_key = (
            unpack_keys.recipient_private_key.kid, extract_key(unpack_keys.recipient_private_key)
        )
        try:
            jwe = JsonWebEncryption()
