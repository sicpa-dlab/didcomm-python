...
from didcomm.pack import pack_encrypted, pack_signed, pack_plaintext
...
async def test_demo_repudiable_non_authenticated_encryption():
    ...
    plaintext = Plaintext(body={"aaa": 1, "bbb": 2},
                          id="1234567890", type="my-protocol/1.0",
                          frm=ALICE_DID, to=[BOB_DID])
    pack_result = await pack_encrypted(plaintext=plaintext, to=BOB_DID)
    packed_msg = pack_result.packed_msg
    print(f"Sending ${packed_msg} to ${pack_result.service_metadata.service_endpoint}")
...
async def test_demo_non_repudiable_encryption():
    ...
    plaintext = Plaintext(body={"aaa": 1, "bbb": 2},
                          id="1234567890", type="my-protocol/1.0",
                          frm=ALICE_DID, to=[BOB_DID])
    pack_result = await pack_encrypted(plaintext=plaintext, frm=ALICE_DID, sign_frm=ALICE_DID, to=BOB_DID)
    packed_msg = pack_result.packed_msg
    print(f"Sending ${packed_msg} to ${pack_result.service_metadata.service_endpoint}")
...
async def test_demo_signed_unencrypted():
    ...
    plaintext = Plaintext(body={"aaa": 1, "bbb": 2},
                          id="1234567890", type="my-protocol/1.0",
                          frm=ALICE_DID, to=[BOB_DID])
    packed_msg = await pack_signed(plaintext=plaintext, sign_frm=ALICE_DID)
...
async def test_demo_plaintext():
    ...
    plaintext = Plaintext(body={"aaa": 1, "bbb": 2},
                          id="1234567890", type="my-protocol/1.0",
                          frm=ALICE_DID, to=[BOB_DID])
    packed_msg = await pack_plaintext(plaintext=plaintext)
    print(f"Publishing ${packed_msg}")
