...
from .helper import (
    diff_type_objects, gen_fwd_msg, gen_fwd_msg_dict
)
...
@pytest.mark.parametrize(
    "msg_t, exc_t", [
        pytest.param(
            123, DIDCommValueError, id="int"
        ),
        pytest.param(
            _build_mturi(scheme="tcp"), DIDCommValueError, id="scheme"
        ),
        pytest.param(
            _build_mturi(domain="somedomain.org"),
            DIDCommValueError,
            id="domain"
        ),
        pytest.param(
            _build_mturi(prot="someprotocol"),
            DIDCommValueError,
            id="protocol"
        ),
        pytest.param(
            _build_mturi(ver="3.0"),
            DIDCommValueError,
            id="too_old_version"
        ),
        pytest.param(
            _build_mturi(ver="1.9"),
            DIDCommValueError,
            id="too_new_version"
        ),
        pytest.param(
            _build_mturi(msg_t="somemsg"), DIDCommValueError, id="message_t"
        ),
    ]
)
...
@pytest.mark.parametrize(
    "msg_ver", ["2.0", "2.0.1", "2.7.9"], ids=lambda x: f"v{x}"
)
...
@pytest.mark.parametrize(
    "msg", [
        pytest.param(gen_fwd_msg_dict(
            remove=[DIDCommFields.ID]), id="no_id"),
        pytest.param(gen_fwd_msg_dict(
            remove=[DIDCommFields.TYPE]), id="no_type"),
        pytest.param(gen_fwd_msg_dict(
            remove=[DIDCommFields.TYP]), id="no_typ"),
        pytest.param(gen_fwd_msg_dict(
            remove=[DIDCommFields.BODY]), id="no_body"),
        # TODO the cases above better to test in scope of GenericMessage
        pytest.param(gen_fwd_msg_dict(
            update={DIDCommFields.BODY: {}}), id="no_next"),
        pytest.param(gen_fwd_msg_dict(
            remove=[DIDCommFields.ATTACHMENTS]), id="no_attach"
        ),
        pytest.param(gen_fwd_msg_dict(
            update={DIDCommFields.ATTACHMENTS: []}), id="empty_attach"
        ),
    ]
)
