...
from didcomm.message import MessageOptionalHeaders, MessageRequiredHeaders, MessageBody, Message
...
async def test_pack_signed():
    ...
    resolvers_config = ResolversConfig(secrets_resolver, ExampleDIDResolver())

    result = await pack_signed(message, alice_did, resolvers_config)
