...
async def check_find_verification_key_by_kid(expected_ver_method: VerificationMethod, resolvers_config_alice):
    verification_method = await find_verification_key(expected_ver_method.id, resolvers_config_alice)
...
async def test_find_verification_key_unknown_kid(resolvers_config_alice):
    with pytest.raises(DIDUrlNotFoundError):
        await find_verification_key(ALICE_DID + "#unknown-key-1", resolvers_config_alice)
